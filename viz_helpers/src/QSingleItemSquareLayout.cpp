...
QSingleItemSquareLayout::QSingleItemSquareLayout(QWidget* parent, float ratio, int spacing): QLayout(parent) {
    init(ratio, spacing);
}

QSingleItemSquareLayout::QSingleItemSquareLayout(float ratio, int spacing) {
    init(ratio, spacing);
}
...
void QSingleItemSquareLayout::init(float ratio_, int spacing) {
	item = 0;
...
QSize QSingleItemSquareLayout::calculateProperSize(QSize from) const {
	...
	if(from.height() < from.width()) {
        ...
        properSize.setWidth(from.height()*ratio - this->margin());
	}
	else {
        properSize.setWidth(from.width() - this->margin());
        properSize.setHeight(from.width()/ratio - this->margin());
	}
	return properSize;
}
