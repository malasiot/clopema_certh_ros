...
void grasp_point(bool , vector<double>& , vector<double>& , vector<vector<int> >& , vector<vector<Point> >& , vector<vector<bool> >&  , int);
int main(){
    folds f;
    //string str="D:/ipthl/rot/rot16";
    int pict_num=170;
    int start_pict=100;
    int step=5;
    ...
    for (int i=start_pict;i<pict_num;i=i+step){
       ...
       // ssc<<str;
        ...
        th=5;
        detected=f.fold_detector( bgrImage, depthMap, th, grasp_candidate, theta,store, location,current_corner,cx );
        if (detected==true){
    ...
    grasp_point( detected , grasp_candidate, theta ,  store , location , current_corner , cx);
